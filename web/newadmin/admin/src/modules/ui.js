...
export const toast = {
  show: (message, type = 'info') => {
    ...
    el.className = `flex items-center gap-3 px-4 py-3 rounded-lg shadow-xl border animate-in slide-in-from-right-full duration-300 ${
      ...
      'bg-white border-emerald-100 text-zinc-600'
    }`;
...
export const modal = {
  show: (title, content, actions) => {
    ...
    dialog.className = 'bg-white border border-emerald-100 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200';
    ...
    const header = `
      <div class="px-6 py-4 border-b border-emerald-100 flex items-center justify-between">
        ...
        <button class="close-modal p-2 hover:bg-emerald-50 rounded-lg transition-colors text-zinc-400">
    ...
    footer.className = 'px-6 py-4 border-t border-emerald-100 bg-emerald-50/40 flex justify-end gap-3';
    
    const closeBtn = document.createElement('button');
    closeBtn.className = 'px-4 py-2 text-sm font-medium text-zinc-500 hover:text-emerald-700 transition-colors';
    ...
    actions?.forEach(action => {
      ...
      btn.className = `px-4 py-2 text-sm font-medium rounded-lg transition-all ${
        action.variant === 'danger' ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-emerald-600 hover:bg-emerald-700 text-white'
      }`;
