...
const WASTE_VALUATIONS = {
  'Plastik': { pointsPerKg: 100, icon: 'droplet' },
  'Kaleng': { pointsPerKg: 250, icon: 'zap' },
  'Kertas': { pointsPerKg: 50, icon: 'file-text' },
  'Kaca': { pointsPerKg: 100, icon: 'glass-water' },
  'Elektronik': { pointsPerKg: 200, icon: 'cpu' },
  'Lainnya': { pointsPerKg: 90, icon: 'help-circle' }
};
...
window.addEventListener('DOMContentLoaded', async () => {
  try {
    // Try to restore and refresh session from backend using stored auth profile.
    const restoredUser = await integrationManager.restoreAndRefreshSession();
...
function showLoginScreen() {
      ...
      <div class="w-full max-w-sm mx-auto px-4">
        <div class="bg-white rounded-3xl shadow-2xl p-8 space-y-6">
          ...
          <div class="text-center space-y-2">
            ...
            <p class="text-slate-500 text-sm">Masuk dengan username dan email</p>
          </div>
          ...
          <form id="login-form" class="space-y-4">
            <div>
              ...
              <input 
                ...
                placeholder="Contoh: budi_setiawan" 
          ...
          <div class="p-3 bg-blue-50 border border-blue-200 rounded-lg text-blue-700 text-xs font-medium">
            <p>💡 <strong>Catatan:</strong> RFID UID diisi admin setelah akun terdaftar.</p>
            <p class="mt-2 text-xs">Belum punya akun? <a href="#" id="open-register-link" class="text-emerald-600 font-semibold">Daftar di sini</a></p>
          </div>
        </div>
      </div>
    `;
  ...
  if (openRegisterLink) openRegisterLink.addEventListener('click', (e) => {
    e.preventDefault();
    showRegisterScreen();
  });
...
function showRegisterScreen() {
  ...
  // Remove existing register if present
  let reg = document.getElementById('register-screen-wrapper');
  if (!reg) {
    reg = document.createElement('div');
    reg.id = 'register-screen-wrapper';
    reg.className = 'fixed inset-0 bg-gradient-to-br from-emerald-50 to-slate-100 flex items-center justify-center z-50';
      ...
      <div class="w-full max-w-sm mx-auto px-4">
        <div class="bg-white rounded-3xl shadow-2xl p-8 space-y-6">
          <div class="text-center space-y-2">
            ...
            <p class="text-sm text-slate-500">Masukkan username dan email Anda</p>
          </div>

          <form id="register-form" class="space-y-4">
            <div>
              <label class="block text-sm font-semibold text-slate-700 mb-2">Username</label>
              <input type="text" id="reg-username" placeholder="username unik" class="w-full px-4 py-3 border border-slate-200 rounded-xl" required />
            </div>
            <div>
              <label class="block text-sm font-semibold text-slate-700 mb-2">Email</label>
              <input type="email" id="reg-email" placeholder="nama@email.com" class="w-full px-4 py-3 border border-slate-200 rounded-xl" required />
            </div>

            <div class="flex gap-2">
              <button type="submit" id="register-btn" class="flex-1 bg-emerald-600 text-white font-bold py-3 rounded-xl">Daftar</button>
              <button type="button" id="register-cancel" class="flex-1 bg-slate-100 text-slate-700 font-semibold py-3 rounded-xl">Batal</button>
            </div>
          </form>
    ...
    mainContent.appendChild(reg);

    // Event handlers
    const registerForm = document.getElementById('register-form');
    const cancelBtn = document.getElementById('register-cancel');
    registerForm.addEventListener('submit', handleRegister);
    cancelBtn.addEventListener('click', () => reg.classList.add('hidden'));
  }
...
async function handleRegister(e) {
  ...
  if (!username || !email) {
    err.textContent = 'Username dan email wajib diisi';
  ...
  try {
    ...
    // Call backend create user profile
    const registeredUser = await apiClient.registerWithProfile(username, email);

    // Auto-login after successful registration
    await integrationManager.initializeUserSession(username, email);
    await loadUserData(registeredUser.rfid_uid);

    // Close register and login screens
    document.getElementById('register-screen-wrapper')?.classList.add('hidden');
    document.getElementById('login-screen-wrapper')?.classList.add('hidden');
    ...
    err.textContent = error.message || 'Gagal mendaftar';
    err.classList.remove('hidden');
...
async function handleLogin(e) {
  ...
  if (!username || !email) {
    showLoginError('Username dan email tidak boleh kosong');
    return;
  }
  ...
  try {
    ...
    const user = await integrationManager.initializeUserSession(username, email);
    console.log('Login successful:', user);

    // Load user data
    await loadUserData(user.rfid_uid);
    ...
    showLoginError(error.message || 'Login gagal. Periksa username dan email Anda.');
...
function showMainApp() {
  ...
  // Hide login/register overlays
  const loginScreen = document.getElementById('login-screen-wrapper');
  if (loginScreen) loginScreen.classList.add('hidden');
  const registerScreen = document.getElementById('register-screen-wrapper');
...
async function loadUserData(rfidUid) {
  try {
    ...
    state.redeemedRewards = syncedData.redeemedRewards || [];
...
async function handleProfileUpdate(e) {
  ...
  try {
    const refreshedUser = await integrationManager.restoreAndRefreshSession();
    if (!refreshedUser) {
      showError('Sesi tidak ditemukan. Silakan login ulang.');
      return;
    }

    await loadUserData(refreshedUser.rfid_uid);
    renderAll();
    showSuccess('Sinkronisasi berhasil. Data terbaru dari admin sudah dimuat.');
  } catch (error) {
    console.error('Profile sync error:', error);
    showError('Gagal sinkronisasi profil: ' + error.message);
  }
}
...
function renderHomeStats() {
  ...
  document.getElementById('stat-balance').textContent = `${wholeNumber(state.stats.totalPoints)} Pts`;
  document.getElementById('stat-co2').textContent = wholeNumber(state.stats.impact?.co2Saved);

  const heroName = document.getElementById('hero-user-name');
  if (heroName) heroName.textContent = state.profile.name || 'User';

  const heroLevel = document.getElementById('hero-user-level');
  if (heroLevel) heroLevel.textContent = String(state.stats.level || 1);
  ...
  if (heroProgressText) heroProgressText.textContent = `${state.stats.totalPoints || 0} / ${nextLevel} Pts (${levelProgressPercentage}%)`;
  const heroProgressBar = document.getElementById('hero-progress-bar');
  if (heroProgressBar) heroProgressBar.style.width = `${levelProgressPercentage}%`;
  ...
  if (rPoints) rPoints.textContent = `${wholeNumber(state.stats.totalPoints)} Pts`;
...
function recalculateFormEstimates() {
  ...
  if (weightInput && labelVal) {
    ...
    const projectedTotal = Math.round(Number(state.stats.totalPoints || 0) + pointsEst);

    document.getElementById('deposit-points-est').textContent = `+${pointsEst} Pts`;
    document.getElementById('deposit-rupiah-est').textContent = `${projectedTotal.toLocaleString('id-ID')} Pts`;
  }
}
  ...
  weekData.forEach((dayData) => {
    ...
    barWrapper.innerHTML = `
      ...
      <div class="w-full sm:w-10 max-w-[40px] rounded-t-lg bg-emerald-500 hover:bg-emerald-600 transition-all duration-300 cursor-pointer relative shadow-sm" style="height: ${percentHeight}%">
        <span class="absolute top-1 inset-x-0 text-center text-[8px] sm:text-[9px] font-bold text-white opacity-0 group-hover:opacity-100 transition-opacity font-mono">${wholeNumber(dayData.weight)}</span>
      </div>
    `;
...
function updateRecentHistoryHome() {
  ...
  if (subset.length === 0) {
    ...
    emptyRow.innerHTML = '<td colspan="6" class="p-4 text-center text-slate-400">Belum ada riwayat deposit</td>';
  ...
  subset.forEach(item => {
    ...
    row.innerHTML = `
      ...
      <td class="p-4 sm:p-5 text-emerald-600 font-mono font-bold">+${wholeNumber(item.points)}</td>
...
function updateHistoryTable() {
  ...
  if (filtered.length === 0) {
    const emptyRow = document.createElement('tr');
    emptyRow.innerHTML = '<td colspan="7" class="text-center p-8 text-slate-400 font-semibold">Tidak ditemukan riwayat penyetoran</td>';
    tbody.appendChild(emptyRow);
    return;
  }
  ...
  filtered.forEach(item => {
    ...
    row.innerHTML = `
      ...
      <td class="p-4 font-mono text-emerald-600 font-black">+${wholeNumber(item.points)} Pts</td>
...
async function redeemRewardItemAPI(item) {
  ...
  try {
    ...
    await loadUserData(state.profile.rfidUid);
...
function showSuccessTicket(transaction) {
  ...
  if (ticketModal) {
    ...
    document.getElementById('ticket-reward').textContent = `${wholeNumber(state.stats.totalPoints)} Pts`;
